...
class CSHoodDataAI(HoodDataAI.HoodDataAI):
    ...
    def __init__(self, air, zoneId=None):
        hoodId = ToontownGlobals.Zones.SellbotHQ
        if zoneId == None:
            zoneId = hoodId
    ...
    def startup(self):
        ...
        self.testElev0 = DistributedFactoryElevatorExtAI.DistributedFactoryElevatorExtAI(self.air, self.air.factoryMgr, ToontownGlobals.Zones.SellbotFactoryInt, 0, antiShuffle=0, minLaff=mins[0])
        self.testElev0.generateWithRequired(ToontownGlobals.Zones.SellbotFactoryExt)
        self.addDistObj(self.testElev0)
        self.testElev1 = DistributedFactoryElevatorExtAI.DistributedFactoryElevatorExtAI(self.air, self.air.factoryMgr, ToontownGlobals.Zones.SellbotFactoryInt, 1, antiShuffle=0, minLaff=mins[1])
        self.testElev1.generateWithRequired(ToontownGlobals.Zones.SellbotFactoryExt)
        ...
        self.lobbyMgr.generateWithRequired(ToontownGlobals.Zones.SellbotLobby)
        self.addDistObj(self.lobbyMgr)
        self.lobbyElevator = DistributedVPElevatorAI.DistributedVPElevatorAI(self.air, self.lobbyMgr, ToontownGlobals.Zones.SellbotLobby, antiShuffle=1)
        self.lobbyElevator.generateWithRequired(ToontownGlobals.Zones.SellbotLobby)
        ...
        if simbase.config.GetBool('want-boarding-groups', 1):
            ...
            self.boardingParty.generateWithRequired(ToontownGlobals.Zones.SellbotLobby)
        ...
        if simbase.config.GetBool('want-boarding-groups', 1):
            ...
            self.factoryBoardingParty.generateWithRequired(ToontownGlobals.Zones.SellbotFactoryExt)
        destinationZone = ToontownGlobals.Zones.SellbotLobby
        ...
        for sp in self.suitPlanners:
            if sp.zoneId == ToontownGlobals.Zones.SellbotHQ:
                sp.cogHQDoors = extDoorList

        intDoor0 = DistributedCogHQDoorAI.DistributedCogHQDoorAI(self.air, 0, DoorTypes.INT_COGHQ, ToontownGlobals.Zones.SellbotHQ, doorIndex=0)
        intDoor0.setOtherDoor(extDoor0)
        intDoor0.zoneId = ToontownGlobals.Zones.SellbotLobby
        for extDoor in extDoorList:
            extDoor.setOtherDoor(intDoor0)
            extDoor.zoneId = ToontownGlobals.Zones.SellbotHQ
            extDoor.generateWithRequired(ToontownGlobals.Zones.SellbotHQ)
            extDoor.sendUpdate('setDoorIndex', [extDoor.getDoorIndex()])
            self.addDistObj(extDoor)

        intDoor0.generateWithRequired(ToontownGlobals.Zones.SellbotLobby)
