...
class LawbotHQDataAI(HoodDataAI.HoodDataAI):
    ...
    def __init__(self, air, zoneId=None):
        ...
        hoodId = ToontownGlobals.Zones.LawbotHQ
        if zoneId == None:
            zoneId = hoodId
    ...
    def startup(self):
        ...
        self.lobbyMgr.generateWithRequired(ToontownGlobals.Zones.LawbotLobby)
        self.addDistObj(self.lobbyMgr)
        self.lobbyElevator = DistributedCJElevatorAI.DistributedCJElevatorAI(self.air, self.lobbyMgr, ToontownGlobals.Zones.LawbotLobby, antiShuffle=1)
        self.lobbyElevator.generateWithRequired(ToontownGlobals.Zones.LawbotLobby)
        ...
        if simbase.config.GetBool('want-boarding-groups', 1):
            ...
            self.boardingParty.generateWithRequired(ToontownGlobals.Zones.LawbotLobby)
        ...
        makeDoor(ToontownGlobals.Zones.LawbotLobby, 0, 1, FADoorCodes.LB_DISGUISE_INCOMPLETE)
        makeDoor(ToontownGlobals.Zones.LawbotOfficeExt, 0, 0)
        officeIdList = [
        ...
        if simbase.config.GetBool('want-boarding-parties', 1):
            ...
            self.officeBoardingParty.generateWithRequired(ToontownGlobals.Zones.LawbotOfficeExt)
